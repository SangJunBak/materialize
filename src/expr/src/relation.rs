        ...
        let mut keys = match self {
            ...
            Filter { predicates, .. } => {
                ...
                if !input.is_empty() {
                    ...
                    //
                    // TODO: maintain unique keys modulo column equivalence. The
                    // canonicalization above already computes the single key that every
                    // variant reduces to. Reporting that key alongside the equivalence
                    // relation, and canonicalizing in consumers before they match, would
                    // make this expansion unnecessary rather than merely bounded.
