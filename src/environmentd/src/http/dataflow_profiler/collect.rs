...
use super::subscribe::{Mode, Spec, Subscribe};
use super::types::{OpId, OpInfo, WorkerId};
...
impl Collector {
    ...
    async fn connect(
        ...
        mpsc::UnboundedSender<SubscribeMessage>,
        mpsc::UnboundedReceiver<SubscribeMessage>,
    ) {
        // Ok(conn)
        mpsc::unbounded_channel()
    }

    pub async fn subscribe(&mut self, spec: impl Spec, mode: Mode) -> anyhow::Result<()> {
        let id = spec.type_id();
